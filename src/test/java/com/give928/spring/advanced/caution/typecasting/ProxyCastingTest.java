...
class ProxyCastingTest {
    ...
    void jdkProxy() {
        ...
        proxyFactory.setProxyTargetClass(false); // JDK 동적 프록시 - 인터페이스 기반으로 프록시 생성
        ...
        assertThrows(ClassCastException.class, () -> {
            ...
            log.info("castingMemberService = {}", castingMemberService);
        });
    }
    ...
    void cglibProxy() {
        MemberServiceImpl target = new MemberServiceImpl();
        ProxyFactory proxyFactory = new ProxyFactory(target);
        proxyFactory.setProxyTargetClass(true); // CGLIB 프록시 - 구체 클래스를 기반으로 프록시 생성
